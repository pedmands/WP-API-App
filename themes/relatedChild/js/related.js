...
// Get the related posts container
...
function getFeaturedImage( postObject ) {
	...
	if ( 0 === postObject.featured_media ) {
		...
		let imgWidth = featuredObject.media_details.sizes.large.width;
		let imgHeight = featuredObject.media_details.sizes.large.height;
...
function sendRESTquery() {
	console.info( 'sendRESTquery() has been triggered!' );
	fetch ( queryURL )
		.then( response => response.json() )
		.then( postObjects => renderRelatedPosts( postObjects ) )
		.catch( (error) => {
